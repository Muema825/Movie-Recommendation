...
def movie_poster_fetcher(imdb_link):
    ...
    url_data = requests.get(imdb_link, verify=False).text
...
st.set_page_config(
   page_title="AI Group Movie Recommendation System",
)
...
def run():
    ...
    st.title("AI Group Movie Recommendation System")
    st.markdown('''<h4 style='text-align: centre; color: #FFF68F;'>* Data is based on "IMDB 5000 Movie Dataset"</h4>''',
    ...
    if cat_op == category[0]:
        ...
        st.markdown('''<h4 style='text-align: left; color: #FFF68F;'>* Fetching a Movie Posters will take a time."</h4>''',
                    unsafe_allow_html=True)
        if dec == 'No':
